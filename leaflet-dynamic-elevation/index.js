...
L.Control.DynamicElevation = L.Control.extend({
  ...
  addRunner: function (data) {
    ...
    point.append("text")
      ...
      .style("stroke", "none")  // Remove any stroke that might cause the box
      .style("pointer-events", "none")  // Prevent accidental selections
      .text(data.text);
  ...
  addCheckpoint: function (checkpoint) {
    ...
    let tooltip = check.append('text')
      ...
      .text(function (d) {
        var returnText;
        if (d === checkpoint.feature.properties.controlpoint) {
          returnText = checkpoint.feature.properties.controlpoint;
        }
        if (d === checkpoint.feature.properties.name) {
